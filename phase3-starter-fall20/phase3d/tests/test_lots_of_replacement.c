//Lots of page replacement.
//pages=4, children=4, frames= 4, priority=3, iterations=1
...
ChildProc(void *arg)
{
    ...
    alive++;
    USLOSS_Console("Child %d starting\n", childPtr->id);
    result = Sys_SemV(running);
    assert(result == 0);
    for (i = 0; i < ITERATIONS; i++) {
        
	//USLOSS_Console("Child %d waiting on SID %d %p\n", childPtr->id, 
        //           childPtr->child, &childPtr->child);
	result = Sys_SemP(childPtr->child);
	assert(result == 0);
	USLOSS_Console("Child %d writing to %d pages\n", childPtr->id, PAGES);
	for (page = 0; page < PAGES; page++) {
	    sprintf(buffer, fmt, childPtr->id, page);
	    target = (char *) (vmRegion + (page * pageSize));
	    debug3("Child %d writing page %d (0x%p): \"%s\"\n", 
		childPtr->id, page, target, buffer);
	    strcpy(target, buffer);
	}
        P3_PrintStats(&P3_vmStats);
	debug3("Child %d resuming parent\n", childPtr->id);
	result = Sys_SemV(childPtr->parent);
	assert(result == 0);
	USLOSS_Console("Child %d waiting (2)\n", childPtr->id);
	result = Sys_SemP(childPtr->child);
	assert(result == 0);
	USLOSS_Console("Child %d reading %d pages\n", childPtr->id, PAGES);
	for (page = 0; page < PAGES; page++) {
	    sprintf(buffer, fmt, childPtr->id, page);
	    target = (char *) (vmRegion + (page * pageSize));
	    debug3("Child %d reading page %d (0x%p)\n", childPtr->id, page,
		target);
...
P4_Startup(void *arg)
{
    ...
    for (i = 0; i < CHILDREN; i++) {
    	...
    	debug3("Created child sem %d %p for child %d\n", 
    	    children[i]->child, &children[i]->child, i);
    	assert(result == 0);
            sprintf(buffer1, "Parent%d", i);
    	result = Sys_SemCreate(buffer1, 0, &children[i]->parent);
    	debug3("Created parent sem %d for child %d\n", 
    	    children[i]->parent, i);
    	assert(result == 0);
    	result = Sys_Spawn(buffer, ChildProc, (void *) children[i], USLOSS_MIN_STACK * 4, PRIORITY, &pid);
    	assert(result == 0);
    ...
    result = Sys_Wait(&pid, &child);
    assert(result == 0);
    TEST(P3_vmStats.faults, 32);
    TEST(P3_vmStats.pageIns, 16);
    TEST(P3_vmStats.pageOuts, 19);
    PASSED();
    USLOSS_Console("Stats after all %d children which each did a read and a write to %d pages\n", CHILDREN, PAGES);
    Sys_VmShutdown();
    return 0;
}
