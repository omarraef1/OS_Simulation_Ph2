// Run the children randomly
// pages=4, children=2, frames=4, priority=3, iterations=2
...
ChildProc(void *arg)
{
	...
	USLOSS_Console("Child %d waiting\n", childPtr->id);
...
P4_Startup(void *arg)
{
    ...
    result = Sys_Wait(&pid, &child);
    assert(result == 0);
    //TEST(P3_vmStats.faults, );
    PASSED();
    USLOSS_Console("Final stats:\n");
    Sys_VmShutdown();
    return 0;
}
